class Game {
    ...
    bunnyCollision = () => {   
        ...
        //se salga por izquierda y arriba aparezca derecha y abajo 
        /*}else if (this.bunny.x < 0){
            this.bunny.x = canvas.width + this.bunny.w
        }else if (this.bunny.y < 0){
            this.bunny.y = canvas.height + this.bunny.h
    ...
    automaticAddFood = () => {
        if ((this.foodArr.length === 0) || (this.foodArr[this.foodArr.length -1].y > canvas.height / 5) ){
            
            let randomPosition = Math.random() * canvas.width
            let speedCounter = this.speedCounter

            let newFoodApple = new Food(randomPosition, "./images/apple.png", speedCounter) 
            this.foodArr.push(newFoodApple)

               
            let randomPosition1 = Math.random() * canvas.width
            
            if (randomPosition1 > randomPosition && randomPosition1 < randomPosition + newFoodApple.w + 100) {
                randomPosition1 = randomPosition1 + newFoodApple.w
            }

            let newFoodCarrot = new Food(randomPosition1, "./images/carrot.png", speedCounter)
            this.foodArr.push(newFoodCarrot) 

            let randomPosition2 = Math.random() * canvas.width

            if (randomPosition2 > randomPosition1 || randomPosition && randomPosition2 < randomPosition1 + newFoodCarrot.w || randomPosition + newFoodApple) {
    ...
    bunnyFoodCollision = () => {
        this.foodArr.forEach((eachFood, index) => {
            if(eachFood.x < this.bunny.x + this.bunny.w &&
    ...
    bunnyCakeCollision = () => {
        this.cakeArr.forEach((eachCake, index) => {
            if(eachCake.x < this.bunny.x + this.bunny.w &&
